...
const  servicesImgs = [
  {img: img1 , title: 'Customized Tours', description: 'Tailor-made itineraries for solo travelers, groups, and families.'},
  {img: img2 , title: 'Cultural Experiences', description: 'Guided tours to UNESCO sites, local crafts, and traditions.'},
  {img: img4 , title: 'Luxury & Comfort', description: 'High-end accommodations and private transport options.'},
  {img: img3 , title: 'Adventure Travel', description: 'Trekking, desert expeditions, and off-the-beaten-path discoveries.'},
]
const Services = () => {
  ...
  return (
    <div className='flex flex-col md:flex-row md:container md:items-center  md:mx-auto md:px-5 gap-14 md:gap-5 2xl:gap-7 pt-52 min-[360px]:pt-32 md:pt-36 lg:pt-48 pb-20 xl:pt-60 '>
       <div className='container md:w-2/6 mx-auto px-5 md:px-0 space-y-2 sm:space-y-3 md:space-y-4  '>
          <h3 className={`${PoppinFont.className} text-2xl  sm:text-3xl  lg:text-3xl  2xl:text-4xl leading-9  2xl:leading-[65px] font-bold`}>{t("title")}</h3>
          ...
          <div  className='relative mx-auto container px-5 md:px-0 md:pl-5 z-20 py-10 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2  xl:grid-cols-4  gap-3 '>
            {
              servicesImgs.map((items, i) => (
                <div className={`   flex justify-between md:flex-col md:justify-between  items-center shadow-xl    h-auto  shadow-[#00000040] bg-white gap-x-6 px-5 dort:px-10 sm:px-5 md:px-5   py-6`}>
                  ...
                  <h3 className={`${PoppinFont.className} md:mt-2 hidden lg:block xl:mt-3 text-center text-[16px] lg:text-sm font-medium `}>{rawtitle[i]}</h3>
                  <div className='md:text-center md:mt-4 gap-3 flex flex-col  xl:mt-3 '>
                    <h3 className={`${PoppinFont.className} md:mt-2 lg:hidden xl:mt-3  text-[16px] lg:text-xl 2xl:text-2xl font-medium `}>{rawtitle[i]}</h3>
