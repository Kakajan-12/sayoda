        ...
        <Link key={tour.id} href={`/tours/${tour.id}`}>
          <div className="flex flex-col border-[#E6E6E6]">
            <Image
              ...
              height={200}
            />
            <div className="w-full h-full p-5 border-x-2 border-b-2 rounded-b-xl flex flex-col gap-3 justify-between border-[#E6E6E6]">
              <p className={`${PoppinFont.className} font-bold text-sm`}>
                {forTittle(stripHtml(String(getLocalized(tour, "title"))), 40)}
              </p>
              <p
                className={`${QuicksandFont.className}  font-medium text-xs line-clamp-4`}
              >
                {stripHtml(String(getLocalized(tour, "text")))}
              </p>
              ...
              <div className="flex flex-col pt-3 gap-2">
                <p className="flex items-center gap-x-2 text-xs">
                  ...
                  <span
                    className={`${MontserratFont.className} text-xs font-bold`}
                  >
                    {t("price")}
                  </span>{" "}
                  {tour.price}$
                ...
                <p className="flex items-center gap-x-2 text-xs">
                  ...
                  {stripHtml(getLocalized(tour, "lang"))}
                </p>
              </div>
            </div>
          </div>
