...
const GalleryCountry =({ blogId }: GalleryProps) => {
    const [images, setImages] = useState<GalleryImage[]>([]);
    const [modalOpen, setModalOpen] = useState(false);
    const [activeImage, setActiveImage] = useState<GalleryImage | null>(null);

    async function fetchGallery(blogId: number) {
        try {
            const res = await fetch(`${BASE_API_URL}/api/blog-gallery/blog/${blogId}`);
            if (!res.ok) {
    ...
    useEffect(() => {
        async function loadGallery() {
            const data = await fetchGallery(blogId);
            setImages(data);
        }
        loadGallery();
    }, [blogId]);
