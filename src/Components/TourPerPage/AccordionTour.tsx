...
const AccordionTour: React.FC<AccordionTourProps> = ({ tourId }) => {
  ...
  return (
    <div className="container mx-auto px-4 pb-5 lg:pb-16">
      <h2
        className={`text-xl lg:text-2xl 2xl:text-3xl leading-9 2xl:leading-[65px] font-bold ${PoppinFont.className}`}
      >
        {t("itinerary")}
      </h2>
      ...
      <div className="flex flex-col md:flex-row justify-between lg:gap-10 gap-5">
        <div className="flex md:w-3/5 flex-col mt-10 gap-3">
          {itinerary.map((item, i) => (
            <div
              key={item.id}
              className="rounded-lg w-full flex flex-col font-medium justify-between sm:text-xl"
            >
              <div
                onClick={() => forToggle(i)}
              ...
              >
                <IoTodayOutline className="w-5 h-5 mt-1 text-white" />
                <div className="flex w-full justify-between items-center">
                  ...
                  />
                  <IoIosArrowDown
                    className={`w-4 h-4 mt-2 ${toggle === i ? "rotate-180" : ""} text-white transition-all duration-300`}
                  />
                </div>
              </div>
                  ...
                  >
                    <div className="py-5 px-5">
                      <div
                        className="mb-3"
                        dangerouslySetInnerHTML={{
                          __html: getLocalizedText(item, "text"),
                        }}
                      />
                      {item.li?.map((el, idx) => (
                        <div
