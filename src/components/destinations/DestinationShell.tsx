...
/**
 * Обложка и вкладки направления.
 *
 * Вынесено из макета сегмента [country], потому что визовая страница
 * Туркменистана переехала на верхний уровень — /turkmenistan-visa, — а
 * вкладки и шапку ей терять нельзя: по самому частотному запросу человек
 * попадает именно сюда, и отсюда должен быть виден путь к турам.
 */
export default async function DestinationShell({
  locale,
  country,
  children,
}: {
