...
const { setPopular, setTourType, setCategory, setLocation, resetFilters } =
  filterSlice.actions;
...
function ToursPage() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${BASE_API_URL}/api/tours`).then(
        (r) => r.json() as Promise<Tour[]>,
      ),
      fetch(`${BASE_API_URL}/api/tour-category`).then(
        (r) => r.json() as Promise<any[]>,
      ),
      fetch(`${BASE_API_URL}/api/tour-location`).then(
        (r) => r.json() as Promise<any[]>,
      ),
    ])
      .then(([toursData, categoriesData, locationsData]) => {
        ...
        setTourTypes(
          Array.from(
            new Map(toursData.map((t: Tour) => [t.tour_type_id, t])).values(),
          ).map((t) => {
  ...
  const filtered = tours.filter(
    (t) =>
      ...
      (filters.tourType === null ||
        Number(t.tour_type_id) === filters.tourType) &&
      (filters.category === null ||
        Number(t.tour_cat_id) === filters.category) &&
  ...
  const FilterForm = (
    <div className="flex flex-col lg:flex-row justify-between items-center w-full space-y-4 md:space-y-0 md:space-x-4">
      <select
        ...
        onChange={(e) =>
          dispatch(
            setPopular(e.target.value === "" ? null : e.target.value === "1"),
          )
        }
      ...
      <select
        ...
        onChange={(e) =>
          dispatch(setTourType(e.target.value ? Number(e.target.value) : null))
        }
        className="border p-2 rounded-md w-56 h-12"
      >
      ...
      <select
        ...
        onChange={(e) =>
          dispatch(setCategory(e.target.value ? Number(e.target.value) : null))
        }
        className="border p-2 rounded-md w-56 h-12"
      >
      ...
      <select
        ...
        onChange={(e) =>
          dispatch(setLocation(e.target.value ? Number(e.target.value) : null))
        }
        className="border p-2 rounded-md w-56 h-12"
      >
